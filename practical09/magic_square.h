...
int isMagicSquare(int ** square, const int n) {//pointer to a pointer
	...
	rowSum=0;
        colSum=0;
        for (j=0; j<n; j++){
	    rowSum+=square[i][j];//fixed row, iterate over columns
            colSum+=square[j][i];//fixed column, iterate over rows
	}
